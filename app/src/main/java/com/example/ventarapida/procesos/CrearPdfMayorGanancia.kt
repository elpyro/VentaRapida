...
import com.example.ventarapida.ui.ReportesViewModel
...
class CrearPdfMayorGanancia {
    ...
    fun mayorGanancia(
        ...
        fechaInicio: String,
        fechaFin: String,
        listaMayorGanancia: Map<ReportesViewModel.ProductoLLave, Double>,
    ) {
    ...
    private fun crearTabla(listaMasVendidos: Map<ReportesViewModel.ProductoLLave, Double>): TablaInventario {
        val table1 = PdfPTable(2)
        table1.widthPercentage = 100f
        table1.setWidths(floatArrayOf(8f,4f))
        table1.headerRows = 1
        ...
        for ((producto, ganancia) in listaMasVendidos) {
            ...
            cell = PdfPCell()
            setCellFormat(cell, cell_color, ganancia.toString().formatoMonenda()!!)
            table1.addCell(cell)

            totalGanancias=totalGanancias + ganancia
            alternate = !alternate
        }
